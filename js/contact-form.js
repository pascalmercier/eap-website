  ...
  function handleFormSubmit(event) {  // handles form submit without any jquery
    ...
    */
    ...
    if( data.email && !validEmail(data.email) ) {   // if email is not valid show error
      ...
      // Hide any previous email validation errors
      var invalidEmail = form.querySelector(".email-invalid");
      if (invalidEmail) {
        invalidEmail.style.display = "none";
      }
      
      // Store original button text and disable with loading message
      var buttons = form.querySelectorAll("button[type='submit']");
      var buttonTexts = [];
      for (var i = 0; i < buttons.length; i++) {
        buttonTexts[i] = buttons[i].textContent;
        buttons[i].disabled = true;
        buttons[i].textContent = "Envoi en cours...";
      }
      ...
      xhr.onload = function() {
        if (xhr.status === 200 || xhr.status === 0) {
          ...
          // Error - re-enable buttons and restore text
      ...
      xhr.onerror = function() {
        // Network error - re-enable buttons and restore text
